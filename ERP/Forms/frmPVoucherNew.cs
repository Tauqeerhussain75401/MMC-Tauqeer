...
namespace ERP
{
    public partial class frmPVoucherNew : Form
    {
        ...
        void Fillquery()
        {
            ...
            for (int i = 0; i < dtquery.Rows.Count; i++)
            {
                ...
                DataTable dtstatus = Query.getData("select status from voucherdetail where status!=1 and vtype='PV'and  vno='" + dtquery.Rows[i]["VNO"].ToString() + "'");
                if (dtstatus.Rows[0]["status"].ToString() == "0")
                {
                    dgvQuery.Rows.Add(Convert.ToDateTime(dtquery.Rows[i]["VDate"]).ToString("dd MMM yyyy"),
                        "PV-" + dtquery.Rows[i]["VNO"].ToString(),
                ...
                else if (dtstatus.Rows[0]["status"].ToString() == "0" && dtstatus.Rows[0]["status"].ToString() == "2")
                {
                    dgvQuery.Rows.Add(Convert.ToDateTime(dtquery.Rows[i]["VDate"]).ToString("dd MMM yyyy"),
                        "PV-" + dtquery.Rows[i]["VNO"].ToString(),
                        dtquery.Rows[i]["Narration"],
                    ...
                    dgvQuery.Rows[i].DefaultCellStyle = new DataGridViewCellStyle() { SelectionForeColor = Color.Black, SelectionBackColor = Color.Orange, BackColor = Color.Orange };
                }
                else
                {
                    dgvQuery.Rows.Add(Convert.ToDateTime(dtquery.Rows[i]["VDate"]).ToString("dd MMM yyyy"),
                ...
                //DataTable dtEdit = Query.GetData("select editby from voucherdetail where status!=1 and vtype='PV'and  vno='" + dtquery.Rows[i]["VNO"].ToString() + "'");
                //if (dtEdit.Rows[0]["editby"].ToString() != null)
                //{
                //    dgvQuery.Rows.Add(Convert.ToDateTime(dtquery.Rows[i]["VDate"]).ToString("dd MMM yyyy"),
                //        "PV-" + dtquery.Rows[i]["VNO"].ToString(),
                //        dtquery.Rows[i]["Narration"],
                ...
                //        dtquery.Rows[i]["VNO"]
                //        );
                //    dgvQuery.Rows[i].DefaultCellStyle = new DataGridViewCellStyle() { SelectionForeColor = Color.Black, SelectionBackColor = Color.DarkOrange, BackColor = Color.DarkOrange };
                //}
            }
        }
        ...
        //     RowCount = dgvAccount.Rows.Count;
        // }
        ...
        public void FillDetail(string Vno)
        {
            ...
            SupportDocuments.doucmentname = "Payment.tiff";
            if (File.Exists("Payment.tiff")) File.Delete("Payment.tiff");
            dgvAccount.Rows.Clear();
            DataTable dt = Query.Voucher(Vno, Variable.BranchCode, "PV");

            dtrowCount = dt.Rows.Count;

            //txtJVNum.Tag = dt.Rows.Cast<DataRow>().Where(w => Convert.ToDecimal(w["cr"]) > 0).ToArray()[0]["vseq"].ToString();
            //txtBankDesc.Text = dt.Select("vseq = '" + Convert.ToString(txtJVNum.Tag) + "'")[0]["description"].ToString();
            //dt.Rows.RemoveAt(dt.Rows.Cast<DataRow>().Where(w => Convert.ToDecimal(w["cr"]) > 0).Select((s, index) => new { index = dt.Rows.IndexOf(s) }).ToArray()[0].index);
            dt.Rows.Cast<DataRow>().Where(w => Convert.ToDecimal(w["cr"]) > 0).ToList().ForEach(fe => dt.Rows.RemoveAt(dt.Rows.IndexOf(fe)));
            ...
            dtpDate.Value = (DateTime)dt.Rows[0]["Vdate"]; //beneficiary
        ...
        bool Calculate()
        {
            ...
            for (int i = 0; i < dgvAccount.Rows.Count; i++)
            {
                ...
                //if (dgvAccount.Rows[i].Cells[clnCredit.Index].Value != null && dgvAccount.Rows[i].Visible == true && dgvAccount.Rows[i].Cells[clnTitleofAccount.Index].Value != null)
                //    Cr += decimal.Parse(dgvAccount.Rows[i].Cells[clnCredit.Index].Value.ToString());
            }
            txtTotalDebit.Text = Dr.ToString("N2");
        ...
        void AllowNewRow()
        {
            if (dgvAccount.CurrentRow.Index == dgvAccount.Rows[dgvAccount.Rows.Count - 1].Index)
            {
                if (dgvAccount.CurrentRow.Cells[clnTitleofAccount.Index].Value != null &&
                    (dgvAccount.CurrentRow.Cells[clnDebit.Index].Value != null
                    //|| dgvAccount.CurrentRow.Cells[clnCredit.Index].Value != null
                    ))
                    dgvAccount.Rows.Add();

            }
            if (Convert.ToString(dgvAccount.CurrentRow.Cells[clnChequeNo.Index].Value) != "" && rdoCheque.Checked)
            {
                if (dgvAccount.CurrentRow.Index + 1 < dgvAccount.Rows.Count)
                {
                    dgvAccount.Rows[dgvAccount.CurrentRow.Index + 1].Cells[clnChequeNo.Index].Value = Convert.ToInt32(dgvAccount.CurrentRow.Cells[clnChequeNo.Index].Value) + 1;
                }
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (MessageBox.Show("Do you wan't to save changes?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    //dr.Add(0);
                    //cr.Add(dgvAccount.Rows.Cast<DataGridViewRow>().Sum(s => Convert.ToDecimal(s.Cells[clnDebit.Index].Value)));
                    //SubAccount.Add(Convert.ToString(cmbBankName.SelectedValue));
                    //Status.Add(0);
                    //ChequeNo.Add("");
                    if (Convert.ToString(cmbTransCode.SelectedValue) == "")
                    {
                        cmbTransCode.Focus();
                        MessageBox.Show("Transaction Narration required...!");
                        return;
                    }
                    ...
                    for (int i = 0; i < dgvAccount.Rows.Count; i++)
                    {
                        if (dgvAccount[clnTitleofAccount.Index, i].Value != null && dgvAccount[clnDebit.Index, i].Value != null /*&& dgvAccount[clnCredit.Index, i].Value != null*/)
                        {
                            if (((string)dgvAccount[clnTitleofAccount.Index, i].Value == AccountHead.ClientAccount ||
                                (string)dgvAccount[clnTitleofAccount.Index, i].Value == AccountHead.DirectorAccount ||
                                (string)dgvAccount[clnTitleofAccount.Index, i].Value == AccountHead.EmployeeAccount) &&
                                (dgvAccount[clnSubAccountDetail.Index, i].Value == null))
                            {
                                dgvAccount.CurrentCell = dgvAccount.Rows[i].Cells[clnSubAccountDetail.Index];
                                MessageBox.Show("Sub Account required...!");
                                return;
                            }
                            if (rdoCheque.Checked && Convert.ToString(dgvAccount[clnChequeNo.Index, i].Value) == "")
                            {
                            ...
                            Seq.Add(Convert.ToInt16(dgvAccount[clnVseq.Index, i].Value.ToString()));
                            Account.Add((string)dgvAccount[clnTitleofAccount.Index, i].Value);
                            Descr.Add((string)dgvAccount[clnDescription.Index, i].Value);
                    ...
                    if (dr.Sum() != cr.Sum())
                    {
                        MessageBox.Show("Debit And Credit not equal...");
                        return;
                    }

                    string Vno = MSP.Voucher_post1("PV", txtJVNum.Text, dtpDate.Value, cmbTransCode.SelectedValue.ToString(), UserInfo.UserId, Variable.BranchCode, Convert.ToString(cmbBankName.SelectedValue), Seq, Account, Descr, dr, cr, SubAccount, Status, ChequeNo, SlipNo, DocumentChanged, Documentid, (File.Exists("Payment.tiff") ? Scanner.FileToArray("Payment.tiff") : null), txtBeneficiary.Text);
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (txtJVNum.Text != "")
            {
                ...
                try
                {
                    ...
                    string AcountClient = cmbTransCode.Text;
                ...
                if (txtJVNum.Text != "")
                {
                    ...
                    if (MessageBox.Show("Do you wan't to delete this?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        if (Remarks != "")
                        {
                            ...
                            string voucherNum = txtJVNum.Text;
                            tf = MSP.Change_GL_status(voucherNum, "PV", "1", Remarks);
        ...
        private void dgvAccount_CellValidated(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                SubAccountDataSource(e.RowIndex);
            }
            if (e.ColumnIndex >= clnTitleofAccount.Index && e.ColumnIndex < clnDescription.Index)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        SubAccount = Convert.ToString(((DataGridViewComboBoxCell)dgvAccount.Rows[e.RowIndex].Cells[clnSubAccountDetail.Index]).Items.Cast<DataRowView>().Where(w => w[0].ToString() == Convert.ToString(dgvAccount.Rows[e.RowIndex].Cells[clnSubAccountDetail.Index].Value)).ToArray()[0][1].ToString());//[Account[0] == AccountHead.ClientAccount ? 3 : 1].ToString());
                    }
                    catch (Exception ee) { }
                    ...
                    {
                        dgvAccount.Rows[e.RowIndex].Cells[clnDescription.Index].Value = "Payment to " + SubAccount;

                    }
                    else
                    {
                        dgvAccount.Rows[e.RowIndex].Cells[clnDescription.Index].Value = "Payment to " + Account[1];

                    }

                    for (int i = 0; i < dgvAccount.SelectedRows.Count; i++)
                    {
                        //do something
                    }
                    //txtBeneficiary.Text = Convert.ToString(dgvAccount.Rows[dgvAccount.CurrentCell.RowIndex].Cells[clnBeneficiary.Index].Value) == "" ?
                    //   (SubAccount == "" ? Account[1] : SubAccount) : dgvAccount.Rows[dgvAccount.CurrentCell.RowIndex].Cells[clnBeneficiary.Index].Value.ToString();
                    txtBeneficiary.Text = Convert.ToString(dgvAccount.Rows[0].Cells[clnSubAccountDetail.Index].EditedFormattedValue);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message.ToString());
        ...
        private void SubAccountDataSource(int RowInd)
        {
            if (RowInd > -1)
            {
                ...
                try
                {
                    ...
                    if (Convert.ToString(dgvAccount.Rows[RowInd].Cells[clnTitleofAccount.Index].Value) == AccountHead.ClientAccount)//Client
                    {

                        DataTable dt = new DataTable();
                        dgvcbc.DataSource = dt;
                        DataView SortingData = dtClients.DefaultView;
                        SortingData.Sort = "clientid ASC";
                        dtClients = SortingData.ToTable();
                        dgvcbc.DataSource = dtClients;
                        dgvcbc.DisplayMember = "clienttitle";
                        dgvcbc.ValueMember = "clientid";
                        dgvAccount[clnSubAccountDetail.Index, RowInd] = dgvcbc;
                        dgvAccount[clnSubAccountDetail.Index, RowInd].ReadOnly = false;

                    }
                    else if (Convert.ToString(dgvAccount.Rows[RowInd].Cells[clnTitleofAccount.Index].Value) == AccountHead.DirectorAccount)//Director
                    {
                        DataTable dt = new DataTable();
                        dgvcbc.DataSource = dt;
                    ...
                    else if (Convert.ToString(dgvAccount.Rows[RowInd].Cells[clnTitleofAccount.Index].Value) == AccountHead.EmployeeAccount)//Employees
                    {
                        DataTable dt = new DataTable();
                        dgvcbc.DataSource = dt;
                    ...
                    else
                    {
                        DataTable dt = new DataTable();
                        dgvcbc.DataSource = dt;
                        dgvAccount[clnSubAccountDetail.Index, RowInd].Value = null;
                        dgvAccount[clnSubAccountDetail.Index, RowInd].ReadOnly = true;
                    }
                ...
                {
                    MessageBox.Show(ex.Message.ToString());
                    Errors.writeline(ex.Message.ToString(), "Payment_ cmbDebit_SelectedIndexChanged");
                }
            }
        ...
        private void dgvAccount_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //    if (dgv.Columns[colIdx.Value].Index == clnDebit.Index)
            //    {
            //        newValue = Convert.ToDouble(dgvAccount[colIdx.Value, rowIdx.Value].Value);
            ...
            //a = txtBeneficiary.Text;

            //b = Convert.ToString(dgvAccount.Rows[0].Cells[clnDebit.Index].Value);

            //c = dtpDate.Value.ToString("ddMMyyyy");
            //d = chkCrossCheque.Checked == true ? "0" : "1";
            //f = cmbBankName.Text;
            AllowNewRow();
            resetRow = true;
        ...
        private void dgvAccount_RowsAdded(object sender, DataGridViewRowsAddedEventArgs e)
        {
            ...
            dgRowCount = dgvAccount.Rows.Count;
            ...
            //{
            //    dgvAccount.Rows[e.RowIndex].Cells[clnFlag.Index].Value = "0";
            //}
            //else
            //{
            //    dgvAccount.Rows[e.RowIndex].Cells[clnFlag.Index].Value = "1";
            ...
            //{
            //   string r= Convert.ToString(dgvAccount.Rows[i].Cells[clnDebit.Index].Value);
            //}
        ...
        public void DgRowCount()
        {
            if (dgRowCount > dtrowCount)
                dgvAccount.Rows[dgvAccount.CurrentCell.RowIndex].Cells[clnFlag.Index].Value = "0";
            ...
            if (e.KeyCode == Keys.Enter)
            {
                DataTable dt = Query.MasterQueryEC(@"select vd.*,get_accountitle_m(vd.fkaccountid,fkclientcode,fkdirectorid,fkhrid,fkbankid) AS title,
                   (CASE WHEN  trunc(To_Date(vdate,'dd Mon yyyy')) = trunc(To_Date(SYSDATE,'dd Mon yyyy')) THEN 1 ELSE 0 END)  AS editable from Voucherdetail vd 
                    where VNO = '" + txtJVNum.Text + "' AND fkbrcode = '" + Variable.BranchCode + "' AND vtype = 'PV' and status <> 1 AND cr = 0 ORDER BY vseq asc");
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        dgvAccount.Rows[Index].Cells[clnDebit.Index].Value = (dt.Rows[i]["dr"] == DBNull.Value ? null : dt.Rows[i]["dr"].ToString());
                    ...
                    cmbTransCode.SelectedValue = dt.Rows[0]["fktransactionid"].ToString();
                    //Calculate();
                    //dgvAccount.Rows[dgvAccount.Rows.Count - 1].Cells[clnVseq.Index].Value = dgvAccount.Rows.Count.ToString();
                    dgvAccount.RowsAdded += new DataGridViewRowsAddedEventHandler(dgvAccount_RowsAdded);
                    dgvAccount.Rows.Add();
                }
                else
        ...
        private void rdoCash_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (rdoCash.Checked == true)
                {
                    ...
                    dgvAccount.Rows.Cast<DataGridViewRow>().ToList().ForEach(fe => fe.Cells[clnChequeNo.Index].Value = "");
                }
                else
                {
        ...
        private void dgvAccount_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            ////string value1 = dgvAccount.CurrentRow.Cells[0].Value != null ? dgvAccount.CurrentRow.Cells[0].Value.ToString() : "";
            ////txtBeneficiary.Text = value1;
            ...
            //foreach (DataGridViewRow row in dgvAccount.Rows)
            //{
            //    dgvAccount.Rows[0].Cells[6].Value = txtBeneficiary.Text;
            //    foreach (DataGridViewCell cell in row.Cells)
            //    {
            //        if (cell.ColumnIndex == 4) //SET YOUR PARTICULAR COLUMN INDEX HERE!!
            //        {
            ...
            //    string n = strNames;
            //    //dgvAccount.Columns["clnBeneficiary"].Visible = true;
            //}
        ...
        {
            if (dgvAccount.CurrentCell.ColumnIndex == clnTitleofAccount.Index)
                ((ComboBox)e.Control).KeyDown += new KeyEventHandler(frmPVoucherNew_KeyDown);
            else if (e.Control is ComboBox)
                ((ComboBox)e.Control).KeyDown -= new KeyEventHandler(frmPVoucherNew_KeyDown);
        }
        //string KeyDown = "";
        void frmPVoucherNew_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void dgvAccount_EditingControlShowing_1(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
        ...
        {
            dgvAccount.CurrentRow.Cells[dgvAccount.CurrentCell.ColumnIndex].Value = (sender as ComboBox).SelectedValue.ToString();
        }
        double amount = 0;
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                amount = Convert.ToDouble(dgvAccount.Rows[i].Cells[clnDebit.Index].Value);
                string name = Convert.ToString(dgvAccount.Rows[i].Cells[clnBeneficiary.Index].Value);
                string date = dtpDate.Value.ToString("ddMMyyyy");
                string ACpayee = chkCrossCheque.Checked == true ? "0" : "1";
                string bnkName = cmbBankName.Text;

                //  frmReportView.ShowBox("Cheque Print PV", name, amount.ToString(), date, ACpayee, bnkName);


            }
            //foreach (DataGridViewRow row in dgvAccount.Rows)
            //{
            //    string a = txtBeneficiary.Text;
            //    string b = Convert.ToString(dgvAccount.Rows[0].Cells[clnDebit.Index].Value);
            //    string c = dtpDate.Value.ToString("ddMMyyyy");
            //    string d = chkCrossCheque.Checked == true ? "0" : "1";
            //    string f = cmbBankName.Text;

            //    FrmReportView.ShowBox("Cheque Print PV", a, b, c, d, f);

            //}
            //   FrmReportView.ShowBox("Cheque Print PV", txtBeneficiary.Text, Convert.ToString(dgvAccount.Rows[0].Cells[clnDebit.Index].Value), dtpDate.Value.ToString("ddMMyyyy"), chkCrossCheque.Checked == true ? "0" : "1", cmbBankName.Text);
        ...
        private void txtBeneficiary_Leave(object sender, EventArgs e)
        {

            if (dgvAccount.CurrentCell.RowIndex > -1)
                dgvAccount.Rows[dgvAccount.CurrentCell.RowIndex].Cells[clnBeneficiary.Index].Value = txtBeneficiary.Text;
        }

        private void dgvAccount_RowLeave(object sender, DataGridViewCellEventArgs e)
        {
            //txtBeneficiary.Text = Convert.ToString(dgvAccount.Rows[dgvAccount.CurrentCell.RowIndex].Cells[clnBeneficiary.Index].Value) == "" ?
            //   (SubAccount == "" ? Account[1] : SubAccount) : dgvAccount.Rows[dgvAccount.CurrentCell.RowIndex].Cells[clnBeneficiary.Index].Value.ToString() ;
            // txtBeneficiary.Text = Convert.ToString(dgvAccount.Rows[0].Cells[clnTitleofAccount.Index].EditedFormattedValue);
        ...
        private void dgvQuery_CellContextMenuStripNeeded(object sender, DataGridViewCellContextMenuStripNeededEventArgs e)
        {
        ...
        private void dgvAccount_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            ...
            if (dgvAccount.Columns[colIdx.Value].Index == clnDebit.Index)
            {
                oldValue = Convert.ToDouble(dgvAccount[colIdx.Value, rowIdx.Value].Value);
            }


        }

        private void cmbTransCode_SelectedValueChanged(object sender, EventArgs e)
        {
            if (cmbTransCode.Text == "PAYMENT OF  EXPENCES")
            {
                grpWithHeld.Visible = true;
                DataTable dtAcc = Query.DetailAccounts();
                cmbWithHeld.DataSource = dtAcc;
                cmbWithHeld.DisplayMember = "accounttitle";
                cmbWithHeld.ValueMember = "AccountId";
            }
            else
            {
                grpWithHeld.Visible = false;
                dgvAccount.Rows.Clear();
                dgvAccount.Rows.Add();
            }
        }

        private void txttoalAmount_TextChanged(object sender, EventArgs e)
        {


        }

        private void txtWithHeldtoalAmount_Leave(object sender, EventArgs e)
        {
            if (txtWithHeldtoalAmount.Text != "")
            {
                DataTable dtsecp = Query.getData("select withheld,REPLACE(per,'%')/100 AS per from chartofaccounts where aclevel=5 and status=0 and withheld is not null and accountid='" + cmbWithHeld.SelectedValue.ToString() + "'");
                if (dtsecp.Rows.Count > 0)
                {
                    dgvAccount.Rows.Clear();
                    decimal per = Convert.ToDecimal(dtsecp.Rows[0]["per"].ToString());
                    decimal withheldamount = Convert.ToDecimal(txtWithHeldtoalAmount.Text) * per;
                    decimal totalamount = Convert.ToDecimal(txtWithHeldtoalAmount.Text) - withheldamount;
                    string description = dtAcc.Select("accountid = '" + dtsecp.Rows[0]["withheld"].ToString() + "'")[0]["accounttitle"].ToString();
                    dgvAccount.Rows.Add(0, cmbWithHeld.SelectedValue.ToString(), null, "Payment to " + cmbWithHeld.Text, totalamount, "");
                    dgvAccount.Rows.Add(0, dtsecp.Rows[0]["withheld"].ToString(), null, "Payment to " + description, withheldamount, "");
                }
            }
        }

        private void txtWithHeldtoalAmount_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsDigit(e.KeyChar) && !char.IsControl(e.KeyChar) && txtWithHeldtoalAmount.Text.IndexOf('.') > -1)
                e.Handled = true;
        }
