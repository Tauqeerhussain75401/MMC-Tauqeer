...
namespace ERP
{
    public partial class frmRVoucherNew : Form
    {
        ...
        void Fillquery()
        {
            ...
            {
                DataTable dtstatus = Query.getData("select status from voucherdetail where status!=1 and vtype='RV' and vno='" + dtquery.Rows[i]["VNO"].ToString() + "'");
        ...
        public void FillDetail(string Vno)
        {
            ...
            DataTable dt = Query.Voucher(Vno, Variable.BranchCode, "RV");
            ...
            dt.Rows.Cast<DataRow>().Where(w => Convert.ToDecimal(w["dr"]) > 0).ToList().ForEach(fe => dt.Rows.RemoveAt(dt.Rows.IndexOf(fe)));
        ...
        bool Calculate()
        {
            ...
            txtTotalDebit.Text = Cr.ToString("N2");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (MessageBox.Show("Do you wan't to save changes?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    for (int i = 0; i < dgvAccount.Rows.Count; i++)
                    {
                        if (dgvAccount[clnTitleofAccount.Index, i].Value != null && dgvAccount[clnCredit.Index, i].Value != null /*&& dgvAccount[clnCredit.Index, i].Value != null*/)
                        {
                    ...
                    string Vno = MSP.Voucher_post1("RV", txtJVNum.Text, dtpDate.Value, cmbTransCode.SelectedValue.ToString(), UserInfo.UserId, Variable.BranchCode, Convert.ToString(cmbBankName.SelectedValue), Seq, Account, Descr, dr, cr, SubAccount, Status, ChequeNo, SlipNo, DocumentChanged, Documentid, (File.Exists("Receipt.tiff") ? Scanner.FileToArray("Receipt.tiff") : null), "");
                ...
                if (txtJVNum.Text != "" && UserInfo.UserId == "Admin")
                {
                    ...
                    if (MessageBox.Show("Do you wan't to delete this?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        if (Remarks != "")
                        {
                            ...
                            string voucherNum = txtJVNum.Text;
                            tf = MSP.Change_GL_status(voucherNum, "RV", "1", Remarks);
        ...
        private void SubAccountDataSource(int RowInd)
        {
            if (RowInd > -1)
            {
                ...
                try
                {
                    ...
                    else if (Convert.ToString(dgvAccount.Rows[RowInd].Cells[clnTitleofAccount.Index].Value) == AccountHead.DealerAccount)//Dealer
                    {
                        DataTable dt = new DataTable();
                        dgvcbc.DataSource = dt;
