...
namespace ERP.Forms
{
    public partial class XRayReport : Form
    {
        ...
        {
            InitializeComponent();
            FillControls.FillcmbTemplateIndexXRay(cmbTemplate);
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (txtReportNo.Text == string.Empty)
            {
                string sql = "SELECT * FROM xrayreport WHERE slipno = '" + txtSlipNo.Text + "'";
        ...
        void FillQuery()
        {
            ...
            if (UserInfo.UserLevel == "Admin")
            {
                dtQuery = Query.getData("select * from XRayReport ORDER BY Id DESC");
            }
            else
            {
                dtQuery = Query.getData("select * from XRayReport where status = 0 and createdby = '" + UserInfo.UserId + "' ORDER BY Id DESC");
            }
        ...
        void LoadTemplate()
        {
            ...
            DataTable dt = Query.getData("SELECT * FROM DocTemplate WHERE Isecho = 2 and id = '" + Id + "'");
            if (dt.Rows.Count > 0)
            {
        ...
        void FillQuery(DateTime? fromDate = null, DateTime? toDate = null, string patientName = "", string receiptNo = "")
        {
            ...
            string query = "select * from xrayreport where 1=1 ";
        ...
        void print()
        {
            ...
            Reports.XRayReport rpt = new Reports.XRayReport();
            ...
            rpt.SetParameterValue("@Headername", "XRAY REPORT");
        ...
        private void dgvQuery_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string sql = "SELECT * FROM XRayReport WHERE slipno = '" + slipNo + "'";
            DataTable dt = Query.getData(sql);
