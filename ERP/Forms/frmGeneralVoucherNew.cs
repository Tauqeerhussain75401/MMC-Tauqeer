...
namespace ERP
{
    public partial class frmGeneralVoucher2 : Form
    {
        ...
        void Fillquery()
        {
            ...
            for (int i = 0; i < dtquery.Rows.Count; i++)
            {
                dgvQuery.Rows.Add(Convert.ToDateTime(dtquery.Rows[i]["VDate"]).ToString("dd MMM yyyy"),
                    "JV-" + dtquery.Rows[i]["VNO"].ToString(),
       ...
       public void FillDetail(string Vno)
        {
            ...
            DataTable dt = Query.Voucher(Vno, Variable.BranchCode, "JV");

            txtJVNum.Text = dt.Rows[0]["VNO"].ToString();
            dgvAccount.Rows.Clear();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                var titile = dt.Rows[i]["title"].ToString();
                int Index = dgvAccount.Rows.Add(dt.Rows[i]["Vseq"].ToString(), dt.Rows[i]["fkaccountid"].ToString(), dt.Rows[i]["description"].ToString(),
                    ((decimal)dt.Rows[i]["dr"]).ToString("N2"), ((decimal)dt.Rows[i]["cr"]).ToString("N2"), null, dt.Rows[i]["status"].ToString());
                ...
                dgvAccount.Rows[Index].Cells[clnSubAccountDetail.Index].Value = dt.Rows[i]["fkaccountid"].ToString() == AccountHead.ClientAccount ? dt.Rows[i]["fkclientcode"].ToString() :
                                                                                ...
                                                                                dt.Rows[i]["fkaccountid"].ToString() == AccountHead.DirectorAccount ? dt.Rows[i]["fkdirectorid"].ToString() : null;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (MessageBox.Show("Do you wan't to save changes?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    for (int i = 0; i < dgvAccount.Rows.Count; i++)
                    {
                        if (dgvAccount[clnTitleofAccount.Index, i].Value != null && dgvAccount[clnDebit.Index, i].Value != null && dgvAccount[clnCredit.Index, i].Value != null)
                        {
                            if (((string)dgvAccount[clnTitleofAccount.Index, i].Value == AccountHead.ClientAccount ||
                                ...
                                //|| (string)dgvAccount[clnTitleofAccount.Index, i].Value == AccountHead.DealerAccount
                                ) &&
                    ...
                    string Vno = MSP.Voucher_post("JV", txtJVNum.Text, dtpDate.Value, cmbTransCode.SelectedValue.ToString(), UserInfo.UserId, Variable.BranchCode, Seq, Account, Descr, dr, cr, SubAccount, Status);
        ...
        private void frmGeneralVoucher_Load(object sender, EventArgs e)
        {
            try
            {
               ...
               /// if (UserInfo.UserId == "Admin") pnlDate.Enabled = true;
                ...
                FillcmbToPay();
                statusUser.Text = "User Name: " + UserInfo.UserId;
                statusDateTime.Text = "LogIn Time: " + SoftwareInfo.ServerDate;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (DeleteRight == true || UserInfo.UserId == "Admin")
                {
                    ...
                    if (txtJVNum.Text != "")
                    {
                        ...
                        if (MessageBox.Show("Do you wan't to delete this?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            ...
                            if (Remarks != "")
                            {
                                ...
                                string voucherNum = txtJVNum.Text;
                                tf = MSP.Change_GL_status(voucherNum, "JV", "1", Remarks);
        ...
        private void dgvAccount_RowsAdded(object sender, DataGridViewRowsAddedEventArgs e)
        {
            if (dgvAccount.Rows[e.RowIndex].Cells[clnStatus.Index].Value == null)
                dgvAccount.Rows[e.RowIndex].Cells[clnStatus.Index].Value = "0";
        ...
        {
            //FrmUploadOpeningBalance frm = new FrmUploadOpeningBalance();
            //frm.ShowDialog();
        }
        string keytype = null;
        private void txtJVNum_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (keytype == "C")
                {
                    dt = Query.MasterQueryEC(@"select vd.*,get_accountitle_m(vd.fkaccountid,fkclientcode,fkdirectorid,fkhrid,fkbankid) AS title,
                   (CASE WHEN  trunc(To_Date(vdate,'dd Mon yyyy')) = trunc(To_Date(SYSDATE,'dd Mon yyyy')) THEN 1 ELSE 0 END)  AS editable from Voucherdetail vd 
                    where VNO = '" + txtJVNum.Text + "' AND fkbrcode = '" + Variable.BranchCode + "' AND vtype = 'JV' and status <> 1 ORDER BY vseq asc");
                }
                else
                {
                    dt = Query.MasterQueryEC(@"SELECT fkaccountid,vtype,vno,description,cr AS dr,dr AS cr,fkclientcode,fkhrid,fkdirectorid,fkdealercode,fktransactionid FROM voucherdetail WHERE (vtype, vno) IN (('JV', '" + txtJVNum.Text + "')) AND status = 0");
                }
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        dgvAccount.Rows[Index].Cells[clnDescription.Index].Value = description + dt.Rows[i]["description"].ToString();
        ...
        private void txtJVNum_TextChanged(object sender, EventArgs e)
        {
